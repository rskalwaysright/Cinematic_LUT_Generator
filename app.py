...
if page == "Generate LUT":
    ...
    if uploaded_ref:
        ...
        st.image(str(str)(\1), caption="Uploaded Reference Frame", use_column_width=True)
        ...
        with st.spinner("Analyzing colors & generating LUT..."):
            try:
                ...
                with col1:
                    st.image(str(str)(\1)(preview_path), caption="Extracted Palette", use_column_width=True)
                with col2:
...
# ---------------- Apply LUT ----------------
    ...
    if "lut_colors" not in st.session_state:
        ...
        if uploaded_test:
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.image(str(str)(\1), caption="Before (Original)", use_column_width=True)
            with col2:
                st.image(str(str)(\1), caption="After (Cinematic LUT Applied)", use_column_width=True)
